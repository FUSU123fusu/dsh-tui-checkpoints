...
import { createRecorder, familyOf, filterEntries, listCheckpoints, readLinks } from './recorder.js'
...
export function apply(ctx) {
  ...
  const linksPath = join(homedir(), '.dsh', 'checkpoints', 'index', `${workspaceId(cwd)}.links.jsonl`)
  const store = createStore()

  // Fork links (child → parent) power the panel's family filter: after a
  // rewind the live session id changes, and only the link map still connects
  // it to its pre-rewind timeline.
  const linkedForks = new Set()
  const appendLink = (sessionId, parentId) => {
    if (linkedForks.has(sessionId)) return
    linkedForks.add(sessionId)
    try {
      mkdirSync(dirname(linksPath), { recursive: true })
      appendFileSync(linksPath, JSON.stringify({ session: sessionId, parent: parentId }) + '\n')
    } catch {
      // A lost link degrades the family filter to current-session-only.
    }
  }
  ...
  ctx.on('session/created', (session) => {
    ...
    if (sessionId !== undefined) appendLink(sessionId, fork.parentSession)
  ...
  const api = {
    ...
    family: (sessionId) => familyOf(readLinks(linksPath), sessionId),
  ...
  commands.register({
    ...
    handler: async ({ agent } = {}) => {
      const rawId = agent?.session?.id
      const sessionId = typeof rawId === 'string' ? rawId : rawId?.value
      store.set({ sessionId })
      const family = api.family(sessionId)
      const entries = filterEntries(api.list(), family, false)
