...
import { filterEntries } from './recorder.js'
...
export function createSceneComponent({ store, api, t }) {
  ...
  return function CheckpointsScene({ React, ui, close }) {
    ...
    // Default view: only the current session's family (ancestors + rewind
    // branches via recorded fork links); `a` widens to the whole workspace.
    const family = api.family(store.data.sessionId)
    const visible = filterEntries(entries, family, showAll)
    ...
    useInput((input, key) => {
      ...
      if (key.downArrow) return setCursor((c) => Math.min(Math.max(0, visible.length - 1), c + 1))
      if (input === 'a' || input === 'A') {
        setShowAll((v) => !v)
        return setCursor(0)
      }
    ...
    const titleKey = showAll || family === undefined ? 'list-title-all' : 'list-title-mine'
    const rows = [
      h(Text, { key: 'title', bold: true, color: 'text' }, t(titleKey, { count: visible.length })),
      h(Text, { key: 'div', dimColor: true, wrap: 'truncate' }, '─'.repeat(Math.max(8, columns - 2))),
    ]

    const start = Math.max(0, Math.min(cursor - bodyRows + 1, visible.length - bodyRows))
    visible.slice(start, start + bodyRows).forEach((entry, index) => {
